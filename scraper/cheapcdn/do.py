...
class Lifecycle:
    ...
    def marked_servers(self):
        ...
        qs = qs.filter(sweepable=True, choiceable=False).all()
    ...
    def resurrect(self, size=1, image=None, ssh_keys=None):
        ...
        for _ in range(0, size):
            ...
            result.append((server, server.create()))
    ...
    def mark(self):
        qs = models.Node.objects
        node = qs.filter(sweepable=True).order_by('id').first()
        if not node:
            return

        node.choiceable = False
        node.save()

    def sweep(self):
        qs = models.Node.objects
        node = qs.filter(sweepable=True, choiceable=False).first()
        if not node:
            return
